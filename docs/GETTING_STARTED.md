...
| **Scripts** | N/A | Runs migrations + seed at startup; run on-demand via `docker compose run --rm scripts scripts/src/seed_from_csv.py` (or Podman) |

## How to Use Swagger UI

1. **Start the application**: `docker compose up --build` (from repo root; or use Podman)
...
1. **Start the application**: `docker compose up --build` (from repo root)
